...
class PaymentController {
  ...
  async updatePaymentStatus(req, res) {
    try {
      ...
      //
      if (status === 'PAID') {
        const dataObiect = new Date(paidDate);
        //
        if (isNaN(dataObiect.getTime())) {
          return res.status(400).json({
            success: false,
            message: 'Invalid paidDate'
          });
        }
        finalPaidDate = dataObiect;
      }

      const newReceiptNumber = `RCPT-${Date.now()}`;
      //
      const uploadDir = path.join(path.resolve(),'receipts');
      const fileName = `${newReceiptNumber}.pdf`;
      ...
      const paymentDetails = {
        ...
        roomNumber: payment.room ? payment.room.roomNumber : 'N/A'
      }
      //2.5 create pdf
      await generateReceiptPdf(paymentDetails,filePath,newReceiptNumber);
      //2.6
      finalReceiptNumber = newReceiptNumber;
